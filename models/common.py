...
from ultralytics.nn.modules import RepVGGDW

from models.swintransformer import SwinTransformerBlock, window_partition
...
class S4(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, reduction=16):
        ...
        # 状态空间建模部分
        self.state_space = nn.Sequential(
    ...
    def forward(self, x):
        ...
        :return: 输出特征图 [B, C, H, W]
        """
        # 局部卷积特征提取
        conv_out = self.relu(self.bn(self.conv(x)))
        # 状态空间建模
        attention = self.sigmoid(self.state_space(x))

        # 动态加权
        output = conv_out * attention
        return output
...
def INF(B, H, W, device):
    # 在指定设备上生成一个 H 维的全-inf向量，并构成对角矩阵，再扩展成 (B*W, H, H)
    inf_diag = torch.full((H,), float("inf"), device=device)
    inf_matrix = -torch.diag(inf_diag).unsqueeze(0).repeat(B * W, 1, 1)
    return inf_matrix
...
class CrissCrossAttention(nn.Module):
    ...
    def __init__(self, in_dim):
        ...
        # 注意：不必在这里保存INF函数，直接调用即可

    def forward(self, x):
        m_batchsize, _, height, width = x.size()
        device = x.device  # 确保所有后续的操作使用相同的设备

        # 检查输入是否为空，避免 RuntimeError
        if m_batchsize == 0 or width == 0 or height == 0:
            raise RuntimeError(f"Invalid input tensor shape: {x.shape}")

        proj_query = self.query_conv(x)
        # 检查 proj_query 是否为空
        if proj_query.numel() == 0:
            raise RuntimeError(f"proj_query is empty, shape: {proj_query.shape}")

        proj_query_H = proj_query.permute(0, 3, 1, 2).contiguous()
        if m_batchsize * width == 0 or m_batchsize * height == 0:
            raise RuntimeError(f"Invalid reshape dimensions: m_batchsize={m_batchsize}, width={width}, height={height}")
        proj_query_H = proj_query_H.view(m_batchsize * width, -1, height).permute(0, 2, 1)
        ...
        # 调用修改后的 INF 函数，确保无穷大矩阵在同一设备上
        energy_H = (torch.bmm(proj_query_H, proj_key_H) + INF(m_batchsize, height, width, device)).view(
            m_batchsize, width, height, height
        ).permute(0, 2, 1, 3)
        energy_W = torch.bmm(proj_query_W, proj_key_W).view(m_batchsize, height, width, width).permute(0, 2, 1, 3)

        # 后续代码……（比如 softmax 及 attention 融合等）
        # 示例：计算注意力矩阵，并与 proj_value 进行融合
        concat_energy = torch.cat([energy_H, energy_W], dim=-1)
        attention = self.softmax(concat_energy)
        # 假设根据 attention 进行加权（具体细节根据实际需要调整）
...
class FourierAttention(nn.Module):
    def __init__(self, in_channels, out_channels=None, reduction_ratio=16):
        ...
        Args:
            ...
            out_channels (int, optional): 输出通道数，如果为 None，则与 in_channels 相同
            reduction_ratio (int): 缩减比例
        """
        super().__init__()
        ...
        self.fc = nn.Sequential(
            nn.Linear(in_channels * 2, self.reduction_channels),
            nn.ReLU(),
            nn.Linear(self.reduction_channels, in_channels),
            nn.Sigmoid()
        )

        # 1×1卷积将通道数变换为 out_channels
        self.conv_reduce = nn.Conv2d(in_channels, self.out_channels, kernel_size=1)
    ...
    def forward(self, x):
        ...
        # 计算频域的通道注意力
        combined = torch.cat([fft_mag.mean(dim=(2, 3)), fft_phase.mean(dim=(2, 3))], dim=1)
        weights = self.fc(combined).view(B, C, 1, 1)

        # 对频域特征进行加权
        fft_weighted = fft_feat * weights

        # 逆傅里叶变换
        x_out = torch.fft.irfft2(fft_weighted, s=(H, W), norm='ortho')

        # 1×1卷积调整通道数
        x_att = x + x_out
        return self.conv_reduce(x_att)
...
class PFMLP(nn.Module):
    def __init__(self, in_ch, dims, layers, block, expand_ratio=1.0, mlp_ratio=1.0,
                 use_dw=False, drop_path_rate=0., num_classes=26):
        """
        Args:
            in_ch (int): 输入通道数，用于第一层卷积
            dims (int): 基础通道数，如果为 int，则内部自动构造各阶段通道列表
            layers (list[int]): 每个 stage 的 block 数量
            block: 使用的基本模块（例如 BottleNeck）
            num_classes (int): 输出通道数/类别数
        """
        ...
        # 修改第一层卷积，使用 in_ch 而非固定的 3
        self.first_conv = ConvX(in_ch, dims[0], 1, 3, 2, use_act=True)
    ...
    def forward(self, x):
        # 如果输入为 dummy forward（batch==1 且空间尺寸为1x1），临时将所有 BN 模块切换为 eval 模式
        if x.shape[0] == 1 and x.shape[2] == 1 and x.shape[3] == 1:
            # 保存当前状态
            training_state = self.training
            # 定义一个函数，设置 BN 模块为 eval
...
class FourierAttention_PFMLP(nn.Module):
    def __init__(self, in_channels, out_channels=None, reduction_ratio=16):
        """
        Args:
            in_channels (int): 输入特征的通道数
            out_channels (int, optional): 输出通道数，如果为 None 则与 in_channels 相同
            reduction_ratio (int): 缩减比例，用于 PFMLP 内部通道数缩减
        """
        super(FourierAttention_PFMLP, self).__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels if out_channels is not None else in_channels
        self.reduction_ratio = reduction_ratio

        # 计算 PFMLP 内部基础通道数，利用 reduction_ratio 缩减：保证至少8个通道
        reduced_dim = max((2 * in_channels) // reduction_ratio, 8)
        # 用 PFMLP 替换原来的 MLP，用于计算通道注意力权重
        # 输入为拼接后的特征（幅度和相位均值），通道数为 2*in_channels
        # 输出需要为 in_channels
        self.fc = PFMLP(
            in_ch=2 * in_channels,   # PFMLP 的第一层卷积输入通道数
            dims=reduced_dim,        # 使用缩减后的基础通道数
            layers=[1, 1, 1, 1],       # 每个 stage 的 block 数量
            block=BottleNeck,         # 使用的基本模块
            num_classes=in_channels   # 输出通道数，与原 MLP 保持一致
        )

        # 1×1 卷积，将经过注意力加权后的特征映射到目标通道数
        self.conv_reduce = nn.Conv2d(in_channels, self.out_channels, kernel_size=1)
    ...
    def forward(self, x):
        ...
        # 分别对幅度和相位进行空间均值池化，得到形状 (B, C)
        mag_mean = fft_mag.mean(dim=(2, 3))
        phase_mean = fft_phase.mean(dim=(2, 3))
        # 拼接后得到形状 (B, 2*C)
        combined = torch.cat([mag_mean, phase_mean], dim=1)

        # 扩展为 4D 张量，形状 (B, 2*C, 1, 1)，适应 PFMLP 的输入
        combined_reshaped = combined.unsqueeze(-1).unsqueeze(-1)

        # 通过 PFMLP 得到通道注意力权重，输出形状为 (B, in_channels)
        weights = self.fc(combined_reshaped).view(B, C, 1, 1)
        ...
        # 残差连接后，通过 1×1 卷积调整通道数
        x_att = x + x_out
        return self.conv_reduce(x_att)

class FourierUnit(nn.Module):
    """频域特征处理模块，输入输出同形 [B, C, H, W]"""
    def __init__(self, dim, groups=1, fft_norm='ortho'):
        super().__init__()
        self.groups   = groups
        self.fft_norm = fft_norm

        # 频域实部+虚部拼接后 channel = 2*dim
        self.conv_layer = nn.Conv2d(
            in_channels=dim * 2, out_channels=dim * 2,
    ...
    def forward(self, x):
        ...
        # 1) FFT 到频域：得到形状 [B, C, H, W//2+1] 的 complex 张量
        fft_feat = torch.fft.rfft2(x, norm=self.fft_norm)

        # 2) 拆分实部/虚部并拼接到 channel
        real = fft_feat.real    # [B, C, H, W//2+1]
        imag = fft_feat.imag    # [B, C, H, W//2+1]
        fft_cat = torch.cat([real, imag], dim=1)  # [B, 2C, H, W//2+1]

        # 3) 1×1 卷积 + 激活
        fft_conv = self.conv_layer(fft_cat)       # [B, 2C, H, W//2+1]
        fft_act  = self.act(fft_conv)

        # 4) 重组回 complex 形式
        fft_act = fft_act.view(B, C, 2, H, -1)             # [B, C, 2, H, W//2+1]
        fft_act = fft_act.permute(0,1,3,4,2).contiguous()  # [B, C, H, W//2+1, 2]
        # 还原 complex 张量
        fft_complex = torch.complex(fft_act[...,0], fft_act[...,1])

        # 5) 逆傅里叶变换
        x_out = torch.fft.irfft2(fft_complex, s=(H, W), norm=self.fft_norm)
